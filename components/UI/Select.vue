...
watch(
	() => selectValue.value,
	(newValue) => {
		if (props.retrieveKey && newValue !== "") {
			props.retrieveKeyCallback(selectItemRef.value[props.retrieveKey]);
			emits("update:modelValue", selectItemRef.value[props.retrieveKey]);
...
<template>
	...
	>
		...
		<!-- Custom Selection -->
		<div
			:class="[inputClass, !options.leftIcon && '!pl-6', !options.rightIcon && '!pr-6']"
		...
		>
			...
			>
				...
				<component
					...
					@click="
						() => {
							selectValue = selectKey ? selectItem[props.selectKey] : selectItem;
