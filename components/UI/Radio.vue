...
const props = defineProps({
	...
	color: {
		type: String,
		validator: (val) => ["green", "blue", "red", "gray", "amber", "gold"].includes(val),
		default: "gray",
	},
	checked: Boolean,
	disabled: Boolean,
		...
		"mr-2 flex h-6 w-6 items-center justify-center rounded-full border-2 border-black bg-white transition-colors duration-150 ease-in-out hover:cursor-pointer hover:transition  peer-focus-visible:ring-2 peer-focus-visible:ring-indigo-500 peer-focus-visible:ring-offset-2 peer-focus-visible:transition",
		{
			variants: {
				color: {
					...
					gray: "border-black/50 peer-checked:bg-zinc-700 peer-hover:border-zinc-700",
					amber: "border-amber-600 peer-checked:bg-amber-500 peer-hover:border-amber-600",
					gold: "border-yellow-600 peer-checked:bg-yellow-500 peer-hover:border-yellow-600",
				},
				disabled: {
...
<template>
	<div class="p-2" :class="props.class">
