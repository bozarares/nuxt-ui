...
const props = defineProps({
	...
	type: String,
	name: String,
	search: { type: Object, default: { items: [], keys: [], hotReload: false } },
	options: {
		type: Object,
		default: () => ({}),
	},
...
const itemToIndex = new Map(props.search.items.map((item, index) => [item, index]));
...
const searchResults = computed(() => {
	...
	if (inputTextValue.value === "") {
		let results = props.search.items.slice(0, maxResults + selectArray.value.length);
		return results
			.map((item) => {
				return {
					item: item,
...
const isExactMatchFound = computed(() => {
	if (searchResults.value[0] && searchResults.value[0].item === inputTextValue.value) {
		return true;
	}
});
	...
	>
		...
		>
			<span
				...
				:class="[
					isLabelFloating ? '-translate-y-4 transform text-xs' : 'transform-none',
		...
		>
			...
			<div
				v-if="isSelectVisible"
				class="m-0.5 flex h-7 w-auto grow-0 basis-6 items-center justify-center gap-0.5 rounded-sm bg-zinc-700 px-1 text-nowrap text-sm text-white"
				v-for="(item, index) in selectArray"
				:key="item"
			>
				{{ item }}
				<button
					class="h-auto text-nowrap w-auto stroke-white pl-0.5 text-white"
					@click="selectArray.splice(index, 1)"
				>
					<component class="h-5 w-5" :is="options.xMarkIcon"></component>
				</button>
			</div>
			<div
				v-if="isSelectVisible"
			...
			>
				<component class="h-full stroke-black/20 font-medium" :is="options.chevronUpIcon" />
			</div>
		</div>
	</div>
</template>
